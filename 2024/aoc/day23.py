        ...
        for computer in self.connections:
            ...
            for neighbor in neighbors:
                for second_neighbor in neighbors:
                    if second_neighbor != neighbor and second_neighbor in self.connections[neighbor]:
    ...
    def find_fully_interconnected_sets(self):
        def is_fully_interconnected(computers):
            for comp in computers:
                if not all(other in self.connections[comp] for other in computers if other != comp):
                    return False
            return True
        ...
        for computer in self.connections:
            if computer not in visited:
                ...
                for other in self.connections[computer]:
                    if other not in visited and is_fully_interconnected(current_set | {other}):
                        current_set.add(other)
                        visited.add(other)

                if len(current_set) > 1:  # Only consider sets with more than one computer
